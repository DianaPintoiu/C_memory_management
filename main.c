...
//la fel ca find dar fara if 
//adica se vor afisa toate elementele din vector
...
int main()
{
	...
	void *arr = NULL;
	int len_arr = 0, len = 0;
	 // vectorul unde citesc linia de la tastatura
	char *citesc = malloc(256 * sizeof(char));
	int cat_timp = 1;
	data_structure *structura = malloc(sizeof(data_structure));

	while (cat_timp == 1) //i break when the command exit is made 
	{
		fgets(citesc, 256, stdin);
		citesc[strlen(citesc) - 1] = '\0'; //terminator de sir
		 //primul cuvant pe care il compar cu comenzile pentru a intra in functie
		char *p = strtok(citesc, " \0\n");

		if (strcmp(p, "insert") == 0)
		{
			// insert
			len = 0; //lenul vectorului data este 0 initial
			structura->header = malloc(sizeof(head)); //alocare pt header
			p = strtok(NULL, " \0\n");
			...
			//copiez in structure data 
			memcpy(structura->data, p, len);
			p = strtok(NULL, " \0\n");
			if (structura->header->type == '1') //copiez biti necesri pt tipul respectiv
			{
				len = len + 1;
			...
			else if (structura->header->type == '2') //copiez biti necesri pt tipul respectiv
			{
				len = len + 2;
				structura->data = realloc(structura->data, len * sizeof(char));
			...
			else if (structura->header->type == '3') //copiez biti necesri pt tipul respectiv
			{
				len = len + 4;
				structura->data = realloc(structura->data, len * sizeof(char));
		...
		else if (strcmp(p, "insert_at") == 0) //la fel ca la insert simplu
		/// + indexul care il extragi primul
		{
			// insert_at
			int index;
			p = strtok(NULL, " \0\n");
			index = atoi(p); //extragere index 
			len = 0;
		...
		{
			// exit
			//eliberarea toatei memorii deoarece s a terminat 
			free(structura);
			free(arr);
			free(citesc);
			//iese din while
